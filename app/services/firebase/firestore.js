...
export const updateUserDisplayName = async ({ id, name }) =>
  usersRef.doc(id).set({ displayName: name }, { merge: true })
...
export const createGame = async ({ sport, place, time, spots }) => {
  ...
  return gamesRef.add({ sport, place, time, spots, ownerId: userId, players: [userId] })
}
